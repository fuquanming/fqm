...
import com.fqm.framework.mq.config.EmqxProperties;
...
public class EmqxClient {
     ...
     */
    public EmqxClient(EmqxProperties properties) throws MqttException {
        this.properties = properties;
        // 发消息，clientId= mac地址+进程pid
        String clientId = NetUtil.getMacAddress(NetUtil.getLocalhost(), "") + ":" + SystemUtil.getCurrentPid();
        client = new MqttClient(properties.getConnectString(), clientId);
    }
    /**
     ...
     * @throws MqttException
     */
    public EmqxClient(EmqxProperties properties, MqttCallback callback, String topic, String port) throws MqttException {
        this.properties = properties;
    ...
    public MqttClient connect() throws MqttException {
        if (null != client) {
            ...
            //设置超时时间，单位为秒
            options.setConnectionTimeout(properties.getConnectionTimeoutSecond());
            //设置心跳时间 单位为秒，表示服务器每隔 1.5*20秒的时间向客户端发送心跳判断客户端是否在线
            options.setKeepAliveInterval(properties.getKeepAliveIntervalSecond());
            //是否自动重新连接
