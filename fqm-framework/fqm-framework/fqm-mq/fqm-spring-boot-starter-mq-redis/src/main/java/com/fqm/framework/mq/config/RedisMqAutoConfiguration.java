...
//                }
                ...
                // Lua获取消费者组
                try {
                    XInfoGroups gs = LuaScriptUtil.getXInfoGroups(v.getTopic());
                    if (gs != null) {
...
//                        stringRedisTemplate.opsForStream().createGroup(v.getDestination(), ReadOffset.from("0"), v.getGroup());
                        LuaScriptUtil.createGroup(v.getTopic(), ReadOffset.from("0"), v.getGroup());
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
                ...
                StreamOffset<String> streamOffset = StreamOffset.create(v.getTopic(), ReadOffset.lastConsumed());
                // 设置 Consumer 监听
                StreamMessageListenerContainer.StreamReadRequestBuilder<String> builder = StreamMessageListenerContainer.StreamReadRequest
                        ...
                        ;
                container.register(builder.build(), new RedisMqListener(v.getBean(), v.getMethod(), stringRedisTemplate, v.getTopic(), v.getGroup()));
            }
        }
        return container;
    }
